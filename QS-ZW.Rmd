...
```{r read-era, dependson = "set-period"}
...
```{r rbind-data, dependson=c("read-era", "read-ncep")}
data <- rbindlist(list(era = era, ncep = ncep),
                  idcol = "dataset")
rm(era, ncep)
```
...
```{r s-timeseries, fig.cap = "Quasi-stationary wave stationarity for wavenumbers 1 to 3 at 50\\degree S and 500hPa for the NCEP Reanalysis and ERA-20C.", fig.width=5, fig.height=7}
pw <- data %>%
...
ggplot(gdata, aes(time, S)) +
  ...
  directlabels::geom_dl(data = gdata[panel == "ncep.-50"],
                        aes(label = k),
                        method = "first.qp") +
  #                          
  # stat_subset(aes(subset = time == max(time),
  #                 # x = time + months(18),
  #                 label = k), geom = directlabels:::geom_dl(), 
  ...
  facet_wrap(~panel, ncol = 1,
             labeller = labeller(panel = c("era.50" = "ERA20C - 50°N",
                                   "era.-50" = "ERA20C - 50°S",
                                   "ncep.50" = "NCEP - 50°N",
                                   "ncep.-50" = "NCEP - 50°S")),
             as.table = FALSE)
...
Because we computed $\hat{S}$ using the whole period, it represents the mean stationarity between `r period_rao[1]` and `r period_rao[2]`. The top row of Figure \ref{fig:s-timeseries} shows how planetary wave stationarity changed for each wavenumber at each latitude according to NCEP. Planetary wave stationarity remained high and constant for wavenumbers 1 to 3 at `r lats`\degree N and 1 at `r lats`\degree S but wavenumbers 2 and 3 at `r lats`\degree S show interdecadal variations.
...
The bottom row of Figure \ref{fig:s-timeseries} shows planetary wave stationarity according to ERA-20C. In the northern hemisphere it agrees well with NCEP during the period of overlap, both in mean value and variations. In the southern hemisphere is is only true for planetary wave 1. ERA-20C slightly overestimates planetary wave 2 stationarity around 1960 by about 40% compared to NCEP. But the most important difference is in planetary wave 3 stationarity. ERA-20C lacks the dramatic change present in NCEP before 1960 and instead has the same mean value and variability before and after that year. The result is that in ERA-20C planetary wave 3 is almost 8 times more stationary than in NCEP before 1960 (Figure \ref{fig:era_over_ncep}). This strongly suggest that the change in NCEP is an artifact of the reanalysis and not a real phenomenon.

```{r era_over_ncep, fig.cap = "ERA-20C to NCEP ratio of planetary wave 3 stationarity at 50\\degree S and 500hPa.", fig.height=2, fig.width = 5}
pw[, .(lat, time, dataset, S, k)] %>% 
  .[k == 3 & lat == -lats] %>% 
  dcast(lat + time + k ~ dataset, value.var = "S") %>% 
  .[, ratio := era/ncep] %>%
  .[complete.cases(.)] %>%
  # .[year(time) > 1960] %>%
  ggplot(aes(time, ratio)) +
  geom_line() +
  scale_y_log10(TeX("\\frac{\\hat{S}_{ERA20C}}{\\hat{S}_{NCEP}}"),
                minor_breaks = NULL, breaks = c(1, 2, 4, 8)) +
  scale_x_datetime("Time", breaks = function(x) pretty(x, 15), 
                   date_labels = "%y",
                   minor_breaks = NULL)
```
