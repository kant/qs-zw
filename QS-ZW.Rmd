...
Some studies analyze the former [e.g. @vanloon1972, @quintanar1995a, @raphael2004b] while others analyze the later [e.g. @rao2004, @turner2017, @irving2015]. For instance, @irving2015 compare their planetary wave activity index with @raphael2004b's wave 3 index and conclude that the later cannot account for events with waves far removed from their climatological position. However, being an index of the zonal wave component in phase with the quasi-stationary wave, this is by design. 
...
A variety of indices that are used in the literature to study zonal waves / planetary waves. @raphael2004b explicitly defined a "zonal wave 3 index" based on averaging the (temporal) anomalies of the three-monthly geopotential height mean at three points that roughly coincide with the position of the climatological zonal wave 3 ridges. @turner2017, on the other hand, used the fourier amplitude of the wave 3. To make these indices more intercomparable between themselves and our data, we replicate them with slight diferences. We compute @raphael2004b baed on monthly means instead of three-monthly means and we compute @turner2017's fourier amplitude at 50S instead of using the mean geopotential height between 55S and 65S.
...
Figure \ref{fig:index-comparison} shows the relationship between the amplitude of the projection of the zonal wave 3 onto the climatological wave 3 and two zonal wave indeces propsed in the literature. @raphael2004b's index is highly correlated with ours (correlation = `r signif(cors$original, 2)`) and the relationship is quite linear in nature. Negative values of @raphael2004b's index are generally associated to zonal waves with negative projection onto the climatological mean. The main difference between @raphael2004b's index is that it consideres temporal anomalies instead of zonal anomalies. Computing the index using zonal anomalies instead increases the correlation to `r signif(cors$zonal, 2)`. 
