...
ggplot(gdata, aes(time, S)) +
  ...
  facet_wrap(~hemisphere, ncol = 1, labeller = labeller(lat = LatLabel),
             as.table = FALSE)
```
...
Figure \ref{fig:index-comparison} shows the relationship between the amplitude of the projection of the zonal wave 3 onto the climatological wave 3 and two zonal wave indeces propsed in the literature. @raphael2004's index is highly correlated with ours (correlation = `r signif(cors$original, 2)`) and the relationship is quite linear in nature. The relationship with @turner2017's approach is very different. While the correlation is not low (`r signif(cors$amplitude, 2)`), but the relationship is far from linear. 

The stark differneces in the shape of the relationship illustrate how 



```{r, raphael-comparison, fig.cap = "",  fig.width=7, fig.height=5}
both %>% 
  .[, .(time, raphael, proj, amplitude)] %>% 
  melt(id.vars = c("time", "raphael")) %>% 
  ggplot(aes(value, raphael)) +
  geom_point(size = 0.4, alpha = 0.5) +
  scale_x_continuous("") +
  scale_y_continuous("ZW3 index from Raphael 2004") +
  stat_rasa(fun = cor_stat, geom = "text", aes(label = paste0("cor = ", stat(cor))),
            xpos = 100, ypos = -2) +
  facet_wrap(~variable, scales = "free",
             labeller = labeller(variable = c(proj = "Amplitude of ZW3 projected into QS3",
                                              amplitude = "Amplitude of ZW3 (Turner et. al. 2017)")))
```


Figure \ref{fig:raphael-comparison} @raphael2004's original zonal wave 3 index with the amplitude of the zonal wave 3 projected onto the climatological wave 3 (left) and with the ZW3 amplitude (right) and illustrates that @raphael2004's index is not a measure of the amplitude of the zonal wave 3, but on its projection onto the climatolgical wave. Moreover, if @raphael2004's index is computed using zonal anomalies instead of the original temporal anomalies, then it becomes almost identical to the projected zonal wave 3 (correlation = `r signif(cors$zonal, 2)`). 
